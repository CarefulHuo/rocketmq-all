...
/**
 * Broker 在 RocketMQ 架构中角色，就是存储消息，核心任务是持久化消息。生产者发送消息给 Broker 消费者从 Broker 消费消息
 */
public class BrokerController {
    ...
    public BrokerController(
        ...
        // 1. 创建 InternalLogger 对象
        // 创建 RebalanceLockManager 对象、List<SendMessageHook>、List<ConsumeMessageHook> 集合
        ...
        /**
         * 2. 创建消息偏移管理器--维护消费者的消息消费进度
         */
        this.consumerOffsetManager = new ConsumerOffsetManager(this);

        /**
         * 3. 创建Topic配置管理器
         *  初始化时，会设置 {@link org.apache.rocketmq.broker.topic.TopicConfigManager#topicConfigTable} 属性
         *  例如：设置自动创建 Topic 时，TBW102 与 TopicConfig 的映射关系
         */
        ...
        /**
         * 9. 上报 Broker 及对应 Topic 的信息到 NameServer
          */
        ...
        // 设置存储 Store 内容的 Broker IP和Port
        this.setStoreHost(new InetSocketAddress(this.getBrokerConfig().getBrokerIP1(), this.getNettyServerConfig().getListenPort()));

        // todo 9. 快速失败的实现类，用于定期检测 PageCache 是否繁忙，繁忙将排队中的发送消息任务线程快速失败掉，结束等待，消费发送者默认会重试 2 次，将消息发送往其他 Broker ，保证高可用
    ...
    public boolean initialize() throws CloneNotSupportedException {
        ...
        // 2. 对 offset 进行加载，将文件内容更新到 org.apache.rocketmq.broker.offset.ConsumerOffsetManager.offsetTable 缓存中
        result = result && this.consumerOffsetManager.load();
        // 3. 对消费者组订阅信息进行加载，将文件内容更新到 org.apache.rocketmq.broker.subscription.SubscriptionGroupManager.subscriptionGroupTable
        result = result && this.subscriptionGroupManager.load();
        // 消费者过滤的相关加载，将文件内容更新到 org.apache.rocketmq.broker.filter.ConsumerFilterManager.filterTable
        result = result && this.consumerFilterManager.load();

        // 以上正常加载之后，开始创建核心对象
        if (result) {
            try {
                // 4.创建 RocketMQ 的核心对象，即消息存储核心类
                this.messageStore =
        ...
        // 6. 加载与恢复文件，主要是在 RocketMQ 启动过程中，根据 CommitLog 重新构建 consumeQueue、index 文件
        ...
        if (result) {
            ...
            // todo 注册各种处理请求(码)的请求处理器，并关联对应的线程池，非常重要
            this.registerProcessor();

            /*---------------------------------------9.定义系列定时线程池-------------------------------*/
            final long initialDelay = UtilAll.computeNextMorningTimeMillis() - System.currentTimeMillis();
            final long period = 1000 * 60 * 60 * 24;
            // Broker 统计线程池，第一次启动的时间为第二天0点，每天执行一次
            ...
            // 每隔 3s 执行一次Broker 的保护逻辑，即存在消费缓慢的消费者时，将消费者组禁用掉，如果打开这个开关的话
            this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
                @Override
                public void run() {
                    try {
