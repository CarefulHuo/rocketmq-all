...
public class SubscriptionData implements Comparable<SubscriptionData> {
    ...
    /**
     * 是否是类过滤模式，默认 false
     * 如果为 true ，表示使用上传过滤器类来进行消息过滤，默认使用 tag 来进行过滤
     */
    ...
    /**
     * 消息过滤表达式
     * 1. 如果 classFilterMode = true 则表示过滤类的全路径名(全限定类名)
     * 2. 如果 classFilterMode = false 则表示按照 tag 或 SQL92表达式，Tag 的情况下，多个用 || 隔开，如 tagA || tagB
     */
    private String subString;

    /**
     * 使用订阅子模式 tag，subString (消息过滤式)中分离的 tag，因为可以使用多个 tag
     * todo 消费端过滤消息时的依据
     */
    private Set<String> tagsSet = new HashSet<String>();

    /**
     * 使用订阅子模式- tag 消息过滤表达式中分离的 tag 的哈希码，因为可以使用多个 tag
     * todo broker端过滤消息时的依据
     */
